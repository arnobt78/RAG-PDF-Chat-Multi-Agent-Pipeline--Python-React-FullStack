...
> **Last verified: 2026-08-08** (web-search + OpenRouter `/api/v1/models` + Groq/Gemini docs on this date). Free tiers change frequently — providers add credit-card requirements, cut rate limits, or deprecate models with little notice. Treat every row below as "true as of last verification," not a permanent guarantee. Re-check a provider's own pricing/docs page before depending on it in production.
>
...
| Provider | Free, no card? | Current free-tier models (verified 2026-08-08) | Notes |
|---|---|---|---|
| **Google AI Studio (Gemini)** 🔑 | **Yes** — genuinely indefinite, no card, no expiration | `gemini-2.5-flash`, `gemini-2.5-flash-lite` (plus rolling `gemini-flash-latest` / `gemini-2.0-flash`). **Pro is paid-only** since April 2026 — do not put `gemini-2.5-pro` on a free fallback chain. Newer Flash-Lite IDs (e.g. 3.x) appear in Google docs; smoke-test before promoting. | OpenAI-compatible endpoint: `https://generativelanguage.googleapis.com/v1beta/openai/`. Free-tier prompts may be used to improve Google's products. Verify live quotas in AI Studio. |
| **GroqCloud** 🔑 | **Yes** — no card | `openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `qwen/qwen3.6-27b` | Extremely fast inference (LPU hardware). See [deprecation watch](#groq-deprecation-watch-verified-2026-08-01-primary-source) — Llama 3.1/3.3 IDs shut down **2026-08-16**. This repo's registry (`backend/app/config.py`) already uses gpt-oss / Qwen. |
| **OpenRouter** 🔑 | **Yes** — no card for `:free` models | Live free set rotates often (~14–18 `:free` text models). As of 2026-08-08 API snapshot: `openrouter/free` (router), `openai/gpt-oss-20b:free`, `google/gemma-4-26b-a4b-it:free`, `nvidia/nemotron-3-nano-30b-a3b:free`, `nvidia/nemotron-3-super-120b-a12b:free`, plus Poolside/Cohere/Nemotron variants. **Meta Llama `:free` tiers largely delisted** — do not depend on `meta-llama/*:free`. | Check `https://openrouter.ai/models?max_price=0` or `GET /api/v1/models` before shipping. Free rate limits: 20 RPM / 50 RPD (higher if you ever add credit). |
...
**rag-pdf-chat registry:** live model chains for this project live in `backend/app/config.py` (`AI_PROVIDERS`) and the frontend fallback `frontend/src/types/index.ts` (`AI_MODELS`). Default chat model: `openai/gpt-oss-20b:free`. Groq uses `openai/gpt-oss-120b` → `openai/gpt-oss-20b` → `qwen/qwen3.6-27b` (no Llama IDs). Sibling portable examples that mention `backend/src/lib/ai/providers.ts` refer to other projects (e.g. CodeBook), not this FastAPI app.
...
**rag-pdf-chat** (this repository) implements the free-tier-first multi-provider chain in Python:

- **`backend/app/config.py`** — `AI_PROVIDERS` registry + `PROVIDER_PRIORITY` + `default_model` (`openai/gpt-oss-20b:free`).
- **`backend/app/services/llm_service.py`** — OpenAI-compatible client with per-provider model failover.
- **`frontend/src/types/index.ts`** — `AI_MODELS` static fallback aligned with the backend registry.
- **`GET /models`** — exposes the live catalog to the model selector.

The portable architecture pattern (registry → generic client → orchestrator) is the reusable idea. Keep keys only in env; never commit DSNs or API keys.
